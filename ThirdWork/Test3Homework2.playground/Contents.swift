...
class Person:CustomStringConvertible{//继承CustomStringConvertible协议，才可print输出对象
    ...
    static func != (lhs: Person, rhs: Person)->Bool{//不相等则相反
        return !(lhs == rhs)
    }
    //description
...
enum Department{
    case organization,publicity,presidium,sports
}
...
class Teacher : Person,SchoolProtocol{
    ...
    func lendBook() {
        print("Teacher \(getFullName()) is lending a book;")
    }
    //继承Person类
    var title:String
    
    init(firstName:String, lastName: String,age :Int,gender:Gender,title:String,department:Department){//构造函数
    ...
    override public func run() {
        print("Teacher \(getFullName()) is running;")
    }
}
...
class Student:Person,SchoolProtocol{
    ...
    func lendBook() {
        print("Student \(getFullName()) is lending a book;")
    }
    //继承Person
    private var stuNo:Int
    init(firstName: String, lastName: String, age: Int, gender: Gender,stuNo:Int,department:Department) {//构造函数
        self.department = department
        self.stuNo = stuNo
        super.init(firstName: firstName, lastName: lastName, age: age, gender: gender)
    }
    ...
    override public func run() {
        print("Student \(getFullName()) is running;")
    }
}
...
//创建三个Teacher对象
let t1 = Teacher(firstName: "me", lastName: "rf", age: 40, gender: Gender.femal, title: "PE", department: Department.sports)
let t2 = Teacher(firstName: "mr", lastName: "ch", age: 38, gender: Gender.male, title: "PC",department:Department.presidium)
let t3 = Teacher(firstName: "mr", lastName: "x", age: 38, gender: Gender.male, title: "MA", department: Department.publicity)

//创建三个Student对象
let s1 = Student(firstName: "st", lastName: "rf", age: 12, gender: Gender.femal, stuNo:22113, department: Department.presidium)
let s2 = Student(firstName: "st", lastName: "cc", age: 14, gender: Gender.male, stuNo:25413, department: Department.sports)
let s3 = Student(firstName: "st", lastName: "am", age: 16, gender: Gender.femal, stuNo:29013, department: Department.sports)
...
for person in personArray {
    person.run()
    if let a = person as? Student{
        a.lendBook()
    }else if let a = person as? Teacher{
        a.lendBook()
    }else{
        print("this person is not a teacher or a student")
    }
}
